...
- Phase 2 ClickHouse：**reqwest 裸 HTTP 传输**（poste-exec 首个 HTTP 传输，Trino 直接复用）。官方 `clickhouse` crate 是 SELECT 取向（拼 ` FORMAT x` + 强制 `readonly=1`）跑不了 DDL——已弃用。验证方式：**mock HTTP 服务器**（Docker Hub 在本网络不可达，见"环境事实"），exec-file/session/introspect 8 类型全过；真实容器验证待网络恢复（命令在"环境事实"）
...
- 实测：`cargo test --workspace` 413 通过（MSSQL 410 + ClickHouse 3）；`diag_mssql.lua` 16/16；`playground/queries/mssql.sql` 19/19 跑两遍
...
### 2. 主线：Phase 3 = Trino（复用 reqwest 传输）

- 设计依据：`dialect-support.md` §一/§四 + 文末触点清单（Lua 9 处、Rust 10 处，动手前照单全查）
- ClickHouse（Phase 2）已建好 HTTP 传输模板：`sql_executor/clickhouse.rs` 的 URL 解析 / basic auth / `default_format=JSON` / session_id 模式可直接套用
- Trino 特有：tpcds 需挂 `etc/catalog/tpcds.properties`；`tpch` 只读，跨引擎核心表进 `memory` catalog（只支持 CTAS、重启即失）；Presto 明确不在范围
- 验收 = `dialect-support.md`"每步完成标准"十条；容器验证前先跑 mock（见"环境事实"）

### 3. Phase 4：DuckDB（可不做）

- `duckdb` crate 链 libduckdb 构建重；文件模式 `duckdb:<path>`，沿 `sqlite:` 前缀先例

### 4. 审计缓办清单（上一轮审计的遗留，按性价比排序）

- 补 `import/format.lua`（`parse_csv` 等，187 行零覆盖，风险最高的测试缺口）
- mssql/clickhouse 专属 completion 关键字/函数——**必须连 Rust `functions.rs` 一起改**，否则撞 drift 测试
...
4. ClickHouse 四连：HTTP 无状态，**同一运行内的多条语句必须共享 `session_id`** 否则临时表不跨语句（exec/session 都已处理）；官方 `clickhouse` crate 是 SELECT 取向不可做通用执行器（已改用 reqwest 裸 HTTP，`default_format=JSON` 的 `meta` 带列类型）；introspect 的 `r[i]` 索引必须走边界安全 `cell()`（mock 抓到过 panic，mssql 同修）；ClickHouse 无事务，`--mode transaction` 直接报错
5. exec-file 的语句切分会剥尾分号、剥 `-- @指令` 行和独立 `###` 行——playground 查询文件分段用 `--` 注释
6. stub-covered 模块（session_conn、introspect、highlights/render、file_exec 等）：给它们加顶层 require 前，先看对应 spec 的 `package.loaded` 桩是否在其 require 之前安装
7. spec 顶层的 `package.loaded` 桩会泄漏到同一 nvim 实例的后续 spec 文件——新 spec 自装自卸
8. Lua 陷阱两连（前置声明、`and f() or f` 非三元）见 AGENTS.md，本仓库都真实咬过人
