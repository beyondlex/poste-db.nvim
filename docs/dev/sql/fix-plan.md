...
- **优先级**：P3
- **问题**：`tests/run.sh:14-20` 无 `-u NONE`，加载用户真实配置；`PLENARY_PATH` 硬编码 lazy 路径；`runtime plugin/poste-db.lua` 带入 `setup()` 全部副作用。
- **方案**：
  1. `tests/run.sh` 改为 `nvim --headless -u tests/minimal_init.lua`（真正替代 vimrc）+ 显式 `runtime plugin/plenary.vim`；`-c "lua require('poste-db.init').setup()"` 显式调用 setup（隔离后的受控副作用）。
...
- **方案（设计）**：
  ...
  2. 若不能改共享设施，则在 poste-db 侧新增 `lua/poste-db/log.lua` 包装层：`M.info(conn_url, ...)` 等，统一在入口做 URL 脱敏后再调 `state.log`。
...
- **涉及文件**：`poste.nvim/lua/poste/state.lua`、`lua/poste-db/{exec_run,sql_runner,file_exec,init,context}.lua`、`lua/poste-db/db_browser/{async,operations}.lua`、`lua/poste-db/session_conn.lua`、新增 `lua/poste-db/log.lua`
...
- **方案（设计）**：
  ...
  3. poste-db 侧所有 `build_cmd`（`exec_run.lua:72-94`、`session_conn.lua:192`、`introspect/table.lua`、`introspect/column.lua`、`db_browser/async.lua`、`file_exec.lua:355`）不再拼 `--connection <url>` 进 argv，改传 env。
- **涉及文件**：`poste.nvim/lua/poste/cli.lua`、`poste.nvim/crates/*`、`lua/poste-db/{exec_run,session_conn,file_exec}.lua`、`lua/poste-db/introspect/{table,column}.lua`、`lua/poste-db/db_browser/async.lua`
...
- **涉及文件**：`lua/poste-db/sql_runner.lua`、`lua/poste-db/session_conn.lua`
...
- **涉及文件**：新增 `lua/poste-db/ident.lua`、`lua/poste-db/db_browser/{operations,actions,schema_create,copy}.lua`、`lua/poste-db/dml.lua`、`lua/poste-db/db_browser/forms_advanced.lua`
...
- **涉及文件**：`lua/poste-db/edit_commit/init.lua`、`lua/poste-db/session_conn.lua`（若需事务透传）、`lua/poste-db/editor/nav.lua`
...
- **涉及文件**：`lua/poste-db/export.lua`、`lua/poste-db/import/format.lua`、`lua/poste-db/import/execute.lua`、`lua/poste-db/import.lua`、`lua/poste-db/import/preview.lua`、`lua/poste-db/import/mapping.lua`、新增 `tests/sql/sql_export_spec.lua`
...
- **涉及文件**：`lua/poste-db/toml.lua`、`lua/poste-db/connections.lua`、新增 `tests/sql/sql_toml_spec.lua`
...
- **涉及**：`lua/poste-db/statement.lua`、`tests/sql/sql_statement_spec.lua`（补 `try_rust_stmt_ranges` 用例，mock `cli.run_json`）
...
- **涉及**：`lua/poste-db/insert_hint.lua`、新增 `tests/sql/sql_insert_hint_spec.lua`（纯解析函数抽取后测：`###` 上方/下方、多 block、光标在 VALUES 内）
...
- **涉及**：`lua/poste-db/constants.lua`、`lua/poste-db/buffer/{init,nav,page,search}.lua`、`lua/poste-db/buffer/nav_raw.lua`
...
- **涉及**：`lua/poste-db/connections.lua:341,359`
...
- **涉及**：`lua/poste-db/connections.lua`、`lua/poste-db/db_browser/{init,actions}.lua`
...
- **涉及**：`lua/poste-db/buffer/init.lua`、`lua/poste-db/sql_runner.lua`、`lua/poste-db/edit_commit/init.lua`、`lua/poste-db/db_browser/actions.lua`
...
- **涉及**：`lua/poste-db/db_browser/async.lua`、`lua/poste-db/db_browser/{init,tree}.lua`、`lua/poste-db/db_browser/actions.lua:115-116,200-201,238-242`、`lua/poste-db/db_browser/operations.lua:294-295`
...
- **涉及**：`lua/poste-db/session_conn.lua`、新增 `tests/sql/sql_session_conn_spec.lua`（mock jobstart 通道测 pending 生命周期）
...
- **涉及**：`lua/poste-db/buffer/nav.lua`、`lua/poste-db/buffer/nav_preview.lua`
...
- **涉及**：`lua/poste-db/sql_runner.lua`、`lua/poste-db/buffer/init.lua`
...
- **涉及**：`lua/poste-db/buffer/search.lua`、`lua/poste-db/buffer/nav_ui.lua:67-74`
...
- **涉及**：`lua/poste-db/buffer/init.lua`
...
- **涉及**：`lua/poste-db/buffer/init.lua`、`lua/poste-db/buffer/header.lua`
...
- **设计（建议）**：
  1. 在 `lua/poste-db/` 新增 `async.lua`（与 `db_browser/async.lua` 区分，或收敛后者）提供：
     ```lua
...
- **涉及文件**：新增 `lua/poste-db/async.lua`、`lua/poste-db/db_browser/async.lua`（收敛）、`lua/poste-db/db_browser/{actions,operations,copy,db_create}.lua`、`lua/poste-db/editor/column.lua`、`lua/poste-db/introspect/init.lua`、`lua/poste-db/source_format.lua`、`lua/poste-db/exec_run.lua`
...
- **涉及文件**：`lua/poste-db/completion/init.lua`、`lua/poste-db/completion/data.lua`、`lua/poste-db/sql_runner.lua`、`lua/poste-db/context_client.lua`（接入或删除）、`lua/poste-db/completion/handlers.lua`
...
- **设计（建议）**：
  ...
  2. **词法感知**：扫描时用简单状态机跳过 `--` 注释、`/* */`、单/双/反引号字符串（复用 `ts_stmt` 或 `insert_hint` 的字符串跟踪逻辑，抽成 `lua/poste-db/lex.lua` 共享）；`USE` 匹配要求语句级（行首 + 非注释内）。
...
- **涉及文件**：`lua/poste-db/context.lua`、新增 `lua/poste-db/lex.lua`、`lua/poste-db/completion/data.lua`、`lua/poste-db/completion/init.lua`、`lua/poste-db/statement.lua`（复用块定位）
...
- **涉及文件**：新增 `lua/poste-db/executor.lua`、`lua/poste-db/sql_runner.lua`、`lua/poste-db/file_exec.lua`、`lua/poste-db/edit_commit/init.lua`、`lua/poste-db/import/execute.lua`
...
- **涉及文件**：`lua/poste-db/dataset.lua`、`lua/poste-db/buffer/init.lua`、`lua/poste-db/buffer/{nav,search,page}.lua`、`lua/poste-db/buffer/nav_ui.lua`、`lua/poste-db/statement.lua`（`extract_label`）、`lua/poste-db/sql_runner.lua`、`lua/poste-db/format.lua`、`lua/poste-db/state.lua`、新增 `lua/poste-db/buffer/history.lua`（侧栏浮动窗）
...
- **涉及文件**：`lua/poste-db/db_browser/{actions,operations,forms,forms_advanced,context_menu,tree,async,init}.lua`、新增 `lua/poste-db/db_browser/util.lua`
...
- **涉及文件**：`lua/poste-db/session.lua`、`lua/poste-db/statement_indicator.lua`、`lua/poste-db/highlights/init.lua`、`lua/poste-db/syntax.lua`、`lua/poste-db/init.lua`、`lua/poste-db/statusline.lua`
- **验收标准**：`require("poste-db.buffer")` 等不再触发高亮/autocmd 注册；测试可独立加载模块。
...
- **涉及文件**：`lua/poste-db/completion/adapter.lua`、`lua/poste-db/completion/init.lua`、`lua/poste-db/init.lua`
...
- **涉及文件**：`lua/poste-db/exec_run.lua`、`lua/poste-db/sql_runner.lua`、`lua/poste-db/session_conn.lua`、`lua/poste-db/format.lua`
...
- `docs/dev/sql/completion/README*`：路径 `lua/poste/sql/` → `lua/poste-db/`，crates 段落改指 poste.nvim；`docs/dev/sql/README.md` 索引补全 10 篇并修正日期；AGENTS.md 修正 `help.lua` 归属与 `LEARNINGS.md` 引用（补文件或删引用）。
