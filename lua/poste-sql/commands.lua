...
function M.setup()
  ...
  vim.api.nvim_create_user_command("PosteDbCmpReload", function()
    ...
    adapter.register_source({ name = "poste_sql", module = "poste-sql.completion", label = "PosteSQL",
          ...
          if col > 0 and line:sub(col, col) == " " then
            ...
            if last_word then
              ...
              if lw == "from" or lw == "join" or lw == "where" or lw == "set" or lw == "on" or lw == "having" or lw == "by" or lw == "and" or lw == "or" then
                vim.schedule(function() pcall(function() require("poste-sql.completion.adapter").show() end) end)
              end
            end
          end
        end,
